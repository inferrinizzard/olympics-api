...
const insertData = async <Row extends Record<string, any>>(
  ...
  return db
    ...
    .finally(() =>
      console.log(`Loaded table ${table} with ${data.length} rows`)
    );
};

const refreshView = (view: string) =>
  db.none(`REFRESH MATERIALIZED VIEW ${view};`);

export const loadData = async () => {
  const countryDetail = loadFile(
  ...
  const countryAthletes = loadFile(
    './json/countryAthletes.json'
  ) as CountryAttendanceRow[];
  ...
  insertData('sports_events', sportsEvents)
    .catch(console.error)
    .then(() => {
      refreshView('country_game_medals').then(() =>
        refreshView('country_medal_totals')
      );
