...
export class SportsDataService {
  ...
  public getAll = (): Promise<SportId[]> =>
    db
      .any(
        `SELECT DISTINCT sport FROM ${this.sportsDetailTable} ORDER BY sport;`
      )
      .then((rows: Pick<SportDetailRow, 'sport'>[]) =>
        rows.map((row) => row.sport)
      );

  public get = (sport: SportId): Promise<SportDetailRow | null> =>
    db.oneOrNone(
      `SELECT * FROM ${this.sportsDetailTable} WHERE sport = '${sport}';`
    );
