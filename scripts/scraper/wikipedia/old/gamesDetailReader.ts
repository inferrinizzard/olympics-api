...
const readGamesInfobox = (infobox: HTMLTableElement): GamesInfoboxData => {
  ...
  for (let i = 1; i < infobox.rows.length; i++) {
    ...
    Object.entries(infoKeys).forEach(
      ([searchKey, { key: dataKey, pattern }]) => {
...
const readCountryAthletes = (html: string) => {
  ...
  const table = [...document.querySelectorAll('table.wikitable')].find(
    (table) => table.querySelector('th')?.textContent?.includes('Participating')
  );
...
export const readGamesDetail = async (
  ...
  return await Promise.all(
    Object.entries(gamesLookup)
      .map(
        ([key, { key: val }]) =>
          [key.replace(/[\[\]]/g, '').split(','), val] as [
            [string, string],
            string
          ] // split '[year,season]' into year & season
      )
      .map(async ([[year, season], gamesKey]) => {
        ...
        const { numAthletes, ...gamesDetails } = readGamesInfobox(
          getInfobox(gamesPageHtml)
        );
        ...
        return {
          ...
          year: parseInt(year),
          season,
          numAthletes: parseInt(
            (numAthletes as string)?.replace(/,/g, '') ?? '-1'
          ),
